...
|---|---|---|---|
| **M1.1** | Arnés de backtesting rolling-origin: cortes mensuales sobre los últimos 18 meses, entrenar ≤ t → predecir t+1..t+12, sin shuffle ni k-fold | S1 | Arnés reproducible con un solo punto de entrada; corridas identificadas |
| **M1.2** | Métricas ADR-008 con `utilsforecast`: WAPE por nivel, MASE vs `SeasonalNaive` por serie, sesgo (%over/under). Cortes de reporte por horizonte (1/3/6/12), **por cuadrante de intermitencia** y por categoría | S1 | Reporte tabular; ningún número global suelto sin desagregar |
