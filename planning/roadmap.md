...
| **Ratificar ADR-015** (el compromiso de precisión se acota por horizonte: punto en h=1/h=3, **intervalo calibrado** en h=6/h=12), 2026-08-05. Es la **invocación explícita del Riesgo 5 del Acta**, que ya pre-autoriza la mitigación ("si a largo plazo la varianza es insalvable, se acotarán las métricas de éxito dándole mayor peso operativo al horizonte de 1 mes"), pero mueve un **criterio de aceptación del R2**, así que la ratifica el PM. Sustento medido sobre el piso real congelado (corrida `a79a9b23676b`): sesgo total **−3,4% (h=1) · −2,6% (h=3) · −5,2% (h=6) · −6,0% (h=12)** — cumple el ±5% en los cortos y lo incumple en los largos. Verificado que **no es efecto de la cobertura** (rellenando el horizonte truncado, h=12 mejora solo a −5,3%). **Ojo con lo que este ADR NO hace: no relaja el gate interno de M2**, que sigue exigiendo que el global corrija el sub-pronóstico largo; acota lo que el producto promete al usuario | **PM** (dueño del criterio de aceptación) + Analista Funcional | ⬜ |
| **Aplicar impacto de ADR-015** a los documentos formales: **Acta** (*Riesgos* 5: registrar que la mitigación se invocó, con fecha y número; hito de validación del R2: criterios **por horizonte**, no un número único); **CU-03** (RNF y paso 4(c): el indicador de confianza pasa a ser el **intervalo P10–P90** — esto **reemplaza al badge de MAPE** que ADR-008 dejó sin dueño; Nota 1 sube de nota a **criterio verificable**); **Plan de Pruebas** (aceptación del R2: ±5% en h=1/h=3, cobertura empírica del intervalo en h=6/h=12); **Matriz de Gestión de Riesgos** (la fila "la métrica de error (RMSE/MAPE) supera el umbral" pasa a WAPE/sesgo por horizonte) | **PM** (Acta, Matriz de Riesgos) + Analista Funcional (CU, Plan de Pruebas) | ⬜ |
| ⚠️ **Revisar ADR-015 ANTES de ratificarlo: su evidencia cambió el mismo día** (2026-08-05, **ADR-016** / M1.9). El sustento de la fila de arriba —sesgo total −5,2% (h=6) y −6,0% (h=12), fuera del ±5%— salió del piso con **selección retrospectiva**. Re-congelado el piso con **selección prospectiva** sobre los mismos datos y los mismos checkpoints (sin reajustar un solo modelo), el sesgo pasa a **+0,8% · +2,0% · −1,0% · −0,9%**: **cumple el ±5% en los cuatro horizontes**. Más de la mitad de la corrección ocurre antes de aplicar la cascada, así que **no es efecto de la cobertura**: el sub-pronóstico de horizonte largo era del criterio de selección, no de los baselines. **Qué se cae:** el argumento del sesgo. **Qué queda en pie y reforzado:** el de la **varianza** — el WAPE a grano producto empeora justamente en horizonte largo (h=6: 0,311 → **0,400**), que es lo que sostiene el intervalo calibrado del punto 2 y el CU-03 del punto 4. El ML Specialist **no modificó ni retiró** ADR-015: le agregó la actualización con el número corregido. La decisión de ratificarlo, reescribirlo o retirarlo es del PM | **PM** (dueño del criterio de aceptación) + Analista Funcional | ⬜ |
