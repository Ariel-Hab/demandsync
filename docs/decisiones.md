...
## ADR-017 — Lo que se promociona es el champion/challenger, no el modelo global solo

**Estado:** **Aceptada (2026-08-06)** — es el criterio de promoción interno del motor, que `motor/plan-diseno.md` §M2 ya fijaba por serie; este ADR lo resuelve con la medición de M2.5 y deja registrado por qué el global solo **no** es promocionable. No toca documentación aprobada ni fronteras entre módulos. **Pero su punto 4 tiene consecuencias fuera del módulo** y esas no las decide el motor.

**Contexto.** §6.5 cerró M2.3 con un titular fuerte: el modelo global le gana al piso de baselines en **11 de 12 celdas** nivel×horizonte. Leído solo, eso sugiere reemplazar los baselines por el global. M2.5 puso a los dos a competir **por serie**, con la regla prospectiva de ADR-016, y el agregado resultó estar tapando dos cosas:

1. **El WAPE agregado es, en un 86%, el cuadrante `suave`.** El WAPE pondera por magnitud y en el catálogo real las magnitudes van de jeringas a vacunas: `suave` carga el **85,9%** de la suma de `|real|`, `erratica` el 13,3%, e `intermitente` más `lumpy` **el 0,6% entre los dos** — aunque son el **31% de los productos**. Un modelo puede errar por 3x en un tercio del catálogo y ganar igual el agregado.
2. **Ahí es exactamente donde el global falla, y empeora con el horizonte.** WAPE a grano producto en `lumpy`: h=1 **1,18** (contra 1,21 del piso, empate), h=12 **5,63** contra **2,55** del piso. En `intermitente` h=12, **3,43** contra 1,23. El modo de falla es **sobre-pronóstico**: el sesgo del global en `lumpy` va de +0,59 (h=1) a **+5,11** (h=12). Tiene sentido para un modelo global —predice la media condicional de productos parecidos, y en una serie que vende a ráfagas esa media está muy por encima del mes típico— pero produce sugerencias de compra inservibles justo en los productos donde el usuario más necesita ayuda.

**Decisión.**

1. **El artefacto promocionable de M2 es el `champion`**: los 9 candidatos —7 baselines + `GlobalLGBM` + `GlobalLGBM_P50`— compitiendo por `(serie, corte)` con selección prospectiva y cascada por disponibilidad (ADR-016), no el global aplicado a todas las series. Es lo que `plan-diseno.md` §M2 ya pedía ("el global ML reemplaza al baseline **solo en las series/niveles donde le gana**"); lo que agrega este ADR es la evidencia de que la alternativa es peor y por qué.
2. **El global solo no cumple el gate de M2.** A h=12 y grano producto da **0,3746 contra 0,3699** del piso: pierde. El champion da **0,3644** y gana. El gate de h=6/h=12 pedía al menos empatar, y el único que empata o gana en los cuatro horizontes es el champion.
3. **`GlobalLGBM_P50` se queda como candidato del champion, pero no se promociona como estimador puntual _global_.** Dos razones, y la primera sola alcanza: (a) **su sesgo viola ADR-008** — a nivel total va de **−8,4% a −13,4%** según el horizonte, fuera del ±5%, porque la mediana de una distribución sesgada a derecha está sistemáticamente por debajo de la media; (b) como candidato **gana 3.512 de 38.014 turnos** de selección —más que `GlobalLGBM`, que gana 2.698— y sin embargo mueve el WAPE del champion **0,0008 a 0,0018**. Compite mucho y aporta casi nada. Esto cierra lo que `roadmap-motor.md` §6.6 punto 4 había dejado abierto: la ventaja del P50 a h=12 era real pero no sobrevive al control de sesgo.

   > **La palabra "global" de arriba no es de relleno: el veredicto lo dicta `suave`, y en los cuadrantes intermitentes el P50 es el mejor de los tres.** En `intermitente` a h=1 da WAPE **0,586** contra 0,960 del piso y 0,960 de la media, **y su sesgo es −9,4% contra +21,1% y +45,7%**: más preciso *y* menos sesgado que las dos alternativas. En `lumpy`, 0,790 contra 1,205 y 1,178, con sesgo **−1,2%** contra +36,0% y +59,1%. Que el sesgo agregado del P50 quede fuera del ±5% es un resultado de `suave` —el 86% del peso—, no una propiedad del estimador en todo el catálogo. **No cambia esta decisión** (el compromiso de ADR-008 se mide al nivel en que se publica), pero sí dice dónde vale la pena volver a mirarlo: la mediana es el estimador natural de una serie que vende a ráfagas, y eso es materia de M3.
4. **El WAPE agregado no alcanza como árbitro de promoción, y eso excede al motor.** Este ADR usa el desagregado por cuadrante **con su peso** para decidir, porque el agregado solo habría promocionado el global. La consecuencia es que el compromiso de precisión del producto —lo que ADR-008 mide y lo que ADR-015 promete— **se cumple de forma muy distinta según el cuadrante**, y eso es lo mismo que M2.4 encontró con la cobertura del intervalo. Decidir si el producto declara una precisión por cuadrante, si acota su promesa, o si CU-03 advierte distinto según la serie, **es del PM y del Analista**; el motor solo aporta la medición. Se suma a las dos entradas que ADR-015 ya tiene pendientes.

**Consecuencias.**

- **El gate de salida de M2 se cumple, con el champion.** Contra el piso prospectivo, a grano producto: **0,3230 / 0,3667 / 0,3928 / 0,3644** contra **0,3305 / 0,3767 / 0,4001 / 0,3699** (h=1/3/6/12) — gana los cuatro. A nivel categoría gana los cuatro. A nivel total gana tres y pierde h=12 por 0,0014. Sesgo a nivel total dentro del ±5% en los cuatro horizontes (−0,31% / +0,77% / −1,49% / −0,82%).
- **Se paga un costo, y hay que decirlo:** en `suave` —el 86% del peso— el champion es **peor que el global solo** (0,2954 contra 0,2654 a h=1). Es varianza de selección: reelegir por corte con evidencia limitada a veces elige mal. El intercambio es perder ~0,03 de WAPE en el 86% del peso para ganar entre 1,5 y 2,9 en el 31% de los productos. Se acepta porque el producto sugiere órdenes de compra **por producto**, y un WAPE de 5,6 en un producto lumpy no es una sugerencia.
- **Los baselines se quedan con el 84% del catálogo.** En pares (serie, corte), las dos variantes del global ganan **6.210 de 38.014 (16,3%)**. `plan-diseno.md` §M2 anticipaba "el baseline se queda con el 30% de las series" como resultado legítimo; el número real es bastante más alto y no cambia la conclusión: el global aporta donde aporta.
- **Sumar candidatos no compra cobertura ni estabilidad.** Las filas sin predicción del champion son **12.700**, exactamente las mismas del piso y del global (altas de catálogo, §5.6.1). Y la estabilidad de la selección no mejora: **335 series (15,7%)** conservan ganador en los 18 cortes con 7 candidatos y **las mismas 335** con 9; la mediana de cambios sube de 6 a 7.
- **Costo de producir la comparación: 45 segundos**, sin reajustar un solo modelo. El `id` de corrida no incluye el predictor, así que las dos corridas ya en disco se cruzan fila a fila (`motor.backtesting.checkpoints`). Rehacerlas costaría 294 min de baselines más 19 de global.
...
- **La sub-cobertura de `erratica` quedó cerrada como irreducible el 2026-08-07, y eso *refuerza* este ADR.** Cuando se escribió, era una deuda abierta sin explicación. Se probaron y descartaron cuatro causas —falta de información (M3.0), post-proceso (§6.9), que el intervalo no escalara con el tamaño del producto, y los hiperparámetros—; la última se midió comparando la cobertura **dentro** de la muestra de entrenamiento contra la de afuera: `erratica` llega a **0,7782 sobre datos ya vistos** (a 2,2 puntos del nominal) y cae a **0,6790** a futuro. O sea que **de los 12 puntos de brecha, ~2 son del modelo y ~10 son que la dispersión futura de una serie errática no está en su pasado**. Detalle en `roadmap-motor.md` §12.0.

  > **Por qué importa para esta decisión:** el punto 2 elegía documentar el 0,679 en vez de prometer 0,80. Con la deuda abierta, eso podía leerse como conformarse con un defecto pendiente de arreglo. Medido, **es informar una propiedad del negocio**: un producto que vende todos los meses en cantidades dispares es menos predecible, y ningún modelo entrenado sobre su propio pasado lo cambia. La honestidad del punto 2 dejó de ser una concesión y pasó a ser el resultado correcto.
