...
- **Si arrancás una sesión en `motor/` o `datasets/`, leé antes `motor/roadmap-motor.md` §12 (deuda conocida y trampas) y `motor/README.md` §Arranque.** El dataset sintético **no está en el repo** (hay que regenerarlo por semilla) y hay tres deudas del generador anotadas como **T0.4**, una de las cuales es precondición de M2.2.
