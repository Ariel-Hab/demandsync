...
- **M1.2 sigue 🟡:** falta el desagregado **por cuadrante de intermitencia**, que depende del clasificador de **M1.4**. `a_markdown()` escribe esa ausencia en el reporte para que no se lea como cumplida. Próximos: **M1.3** (test anti-leakage de deflación) y **M1.4** (que además cierra M1.2 y mueve el clasificador de `datasets/` al motor).
