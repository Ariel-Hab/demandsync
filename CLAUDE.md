...
**Última actualización:** 2026-08-02 — **M1.8b cerrada (ADR-012)**: el extract nunca filtró obsequios ni descontinuados, y la réplica del snap puede estar atrasada sin que nada avise. **El piso real quedó pendiente de re-congelar** — la tabla vigente mide sobre un universo con obsequios y con su último mes (2026-06) al 32% de carga. Sirve de referencia, **no de gate**, hasta re-correrla. Después de eso sigue **M2.2** (features).
...
- **Motor — sigue:** **M2.2 → M2.5** (features → champion/challenger). El extract vive en `C:/dfv-extract` (fuera del repo, son datos reales); regenerarlo requiere el túnel SSH de cotizaciones arriba (`docker compose --profile dev up`) y `pip install -e "motor[extract]"`.
...
- **Antes de M2.2 queda una sola cosa: re-congelar el piso** sobre el universo nuevo (2.128 productos, ventana → 2026-05). Son 214 min. Hasta entonces `baselines-real-2026-07-31.md` es referencia, **no gate**.
