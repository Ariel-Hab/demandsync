...
**Última actualización:** 2026-07-31 — **M1 CERRADO**, **T0.4 CERRADA** y **M2.1 CERRADA**. Piso real congelado en `motor/backtests/baselines-real-2026-07-31.md` (corrida `f7af767ca7e6`, 2.189 productos × 18 cortes, 214 min). Desde acá rige baselines-first: ningún modelo se promociona sin batir esa tabla. Sigue **M2.2** (features).
...
- **Dos cosas de M2.1 para el que siga:** (1) el peldaño **laboratorio lo usa 1 producto** de 2.189 — los datos reales no lo ejercitan, se testea a mano o queda sin cubrir; (2) **el clamp no cubre el deflactor directo**: un precio propio de $0,01 da deflactores de hasta 1,2 M en 93 filas. No mueve el agregado (0,000% del revenue real) y por eso no se tocó, pero **hay que decidirlo antes de M2.2**, que construye features sobre esa columna.
