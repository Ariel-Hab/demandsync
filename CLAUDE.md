...
**Última actualización:** 2026-07-31 — **M1 CERRADO** y **T0.4 CERRADA**. Piso real congelado en `motor/backtests/baselines-real-2026-07-31.md` (corrida `f7af767ca7e6`, 2.189 productos × 18 cortes, 214 min). Desde acá rige baselines-first: ningún modelo se promociona sin batir esa tabla. Sigue **M2.1** (transformador de deflación); la precondición de M2.2 ya está cumplida.
...
- **Motor — sigue:** **M2** (M2.1 deflación → M2.5 champion/challenger). El extract vive en `C:/dfv-extract` (fuera del repo, son datos reales); regenerarlo requiere el túnel SSH de cotizaciones arriba (`docker compose --profile dev up`) y `pip install -e "motor[extract]"`.
